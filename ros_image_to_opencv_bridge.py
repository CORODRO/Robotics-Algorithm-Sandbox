...
class ROS2OpenCV2(object):
    def __init__(self, node_name):        
        ...
        cv.namedWindow(self.cv_window_name, cv2.WINDOW_AUTOSIZE)
        if self.resize_window_height > 0 and self.resize_window_width > 0:
            cv2.resizeWindow(
                self.cv_window_name,
                self.resize_window_width,
    ...
    def image_callback(self, data):
        ...
        if self.show_boxes:
            if self.track_box is not None and self.is_rect_nonzero(self.track_box):
                ...
                if self.face_tracking:
                    ...
                    cv2.rectangle(self.display_image, pt1, pt2, cv2.COLOR_BGR2RGB, self.feature_size, 8, 0)
                else:
                    # Otherwise, display a rotated rectangle
                    vertices = np.int0(cv2.boxPoints(self.track_box))
                    cv2.drawContours(self.display_image, [vertices], 0, cv2.COLOR_BGR2RGB, self.feature_size)
            ...
            # If we don't yet have a track box, display the detect box if present
                ...
                if self.show_boxes:
                    cv2.rectangle(self.display_image, (pt1_x, pt1_y), (pt1_x + w, pt1_y + h), cv2.COLOR_BGR2RGB, self.feature_size, 8, 0)
        ...
        if self.show_text:
            ...
            cv2.putText(self.display_image, "CPS: " + str(self.cps), (10, vstart), font_face, font_scale, cv2. COLOR_BGR2RGB) #cv2. COLOR_BGR2RGB
            cv2.putText(self.display_image, "RES: " + str(self.frame_size[0]) + "X" + str(self.frame_size[1]), (10, voffset), font_face, font_scale, cv2. COLOR_BGR2RGB) #(255, 255, 0)        
